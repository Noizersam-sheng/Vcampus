...
import model.consumption_record.Consumption_record;
import model.courses.Course;
import model.library.Book;
import model.library.Book_borrowing_record;
import model.store.Commodity;
import model.store.Commodity_Car;
import model.store.Commodity_Record;
import model.store.Sales;
import socket3.Client;
...
//   	int tag=(int)ob;
	...
	public Object[][] show_list(String c_id) {
		...
		for (int i = 0; i < D.size(); i++) {
			B = (ArrayList<ArrayList<String>>) a.request(41, true, "id" + "\n" + D.get(i));
			for (int j = 0; j < B.size(); j++) {
				String name = B.get(j).get(3);
				if (B.get(j).get(4).equals("4"))
					D.remove(B.get(j).get(0));
				else
					E.add(name);
			}
		}
