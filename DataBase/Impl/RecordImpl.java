 ...
 * @version 1.0
 */
public class RecordImpl implements BaseDao, StudentDao {
  Connection connection;
  private HashMap<String, String> ways = new HashMap<String, String>();
  ...
  public Integer addUser(String[] args) {
    ...
    try {
      getConnection();
      String sql = "INSERT INTO " + RecordTableName + "(id,ways,cost,surplus,time)" + " VALUES(?,?,?,?,?)";
      ...
      pstmt.setDate(5, stringToSqlDate(time));
  ...
  public ArrayList<ArrayList<String>> searchOneUser(String condition_name, String condition) {
    ...
    try {
      ...
      if (condition_name.equals("null") && condition.equals("null"))
        sql = "SELECT* FROM " + RecordTableName + " ORDER BY time DESC";
      else
        sql = "SELECT* FROM " + RecordTableName + " WHERE " + condition_name + " LIKE" + "'%" + condition + "%'"
      ...
      while (rs.next()) {
        ...
        for (int i = 1; i <= rsmd.getColumnCount(); i++) {
          if (i == 2)
            temp.add(ways.get(rs.getString(i)));
          else
  ...
  public ArrayList<String> searchAllUser(String column) {
    ...
    try {
      getConnection();
      statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      String sql = "SELECT* FROM " + RecordTableName;
      rs = statement.executeQuery(sql);
      while (rs.next()) {
  ...
  public static void main(String[] args) {
   ...
   System.out.println(rl.searchOneUser("null", "null"));
  }
}
