...
public class StoreImpl implements BaseDao, StudentDao {
  ...
  public Integer addUser(String[] args) {
    ...
    try {
      ...
      pstmt.setDate(7, stringToSqlDate(time));
  ...
  public Integer updateUser(String[] args) {
    ...
    try {
      ...
      pstmt.setDate(6, stringToSqlDate(time));
  ...
  public Integer addConsumption(String args[]) {
    ...
    try {
      ...
      String sql = "INSERT INTO " + ConsumptionTableName
          + "(id,name,commodity_id,commodity_name,type,num,cost,time,market) VALUES(?,?,?,?,?,?,?,?,?)";
      pstmt = connection.prepareStatement(sql);
  ...
  public ArrayList<ArrayList<String>> searchAllRecord(String id) {
    ...
    try {
      ...
      String sql = null;
      statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      sql = "SELECT* FROM " + ConsumptionTableName + " WHERE id=" + id;
      rs = statement.executeQuery(sql);
      rsmd = rs.getMetaData();
      while (rs.next()) {
        temp = new ArrayList<String>();
