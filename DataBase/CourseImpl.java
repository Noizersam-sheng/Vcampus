...
public class CourseImpl implements StudentDao, BaseDao {
  ...
  public Integer updateUser(String[] args) {
    ...
    try {
      ...
      String sql = "UPDATE " + CourseTableName
          + " SET course_id=?,course_name=?,teacher_name=?,time=?,start_time=?,end_time=?,status=?,now_num=?,max_num=?,classroom=?,num=?"
   ...
   * 
   * @param cou_name 课程名字
   * @param status_  必/选修 （1为必修，2为选修）
   * @param isMax    该课程是否已满 （1为未满，2为已满）
   * 
   * @return 返回为符合该索引值的所有信息，包括课程id，课程名字等等， 服务端根据客户端的需求自行选择信息返回(注意：下标从0开始！！)；
   *         每一行的信息为（0：课程id，1：课程名称，2：老师名字，3：星期几，4：起始时间，5：结束时间，6：状态，7：现选人数，8：最大人数，9：教室，10：几节课）
   */
  public ArrayList<ArrayList<String>> searchOneUser(String cou_name, String status_, String isMax) {
    ...
    String restrainSql = null;
    if (isMax.equals("2"))
      restrainSql = " WHERE now_num=max_num";
    else
      restrainSql = " WHERE now_num<max_num";
  ...
  public static void main(String[] args) {
    ...
    System.out.println(cl.searchOneUser("数据库", "1", "0"));
  }
}