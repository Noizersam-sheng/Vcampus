...
import model.personnel.PassData;
import model.personnel.Person;
import model.store.Commodity;
import model.store.Commodity_Car;
import model.store.Commodity_Record;
import ui.support.jwc.ClassBlock;
import ui.support.jwc.CourseScrollContent;
import ui.support.jwc.Timetable;
import ui.support.library.JBorrowTable;
import ui.support.library.JCollectionTable;
import ui.support.library.JLibraryManagerTable;
import ui.support.shop.CommoScrollContent;
import ui.support.shop.JCComboBox;
import ui.support.shop.JRedLabel;
...
public class JCButton extends JLabel {
	...
	// icon
	ImageIcon shopcartIcon = new ImageIcon(MyType.iconPath + "shopcart.png");
...
//						new Commodity("", "学习资料", "数据结构笔记", 98, 0, 0.01, "", 1, 2, "213193904", ""), };
		...
		// [start], bottom side
		JRedLabel shopcart = new JRedLabel(" 购物车", shopcartIcon, JLabel.CENTER);
		JRedLabel myCommo = new JRedLabel("我的二手商品");
		JRedLabel myShoppingReport = new JRedLabel("我的消费报告");
		JRedLabel myShoppingDetail = new JRedLabel("我的消费明细");
		JRedLabel exit = new JRedLabel("退出");

		// size & location
		int PIXEL = JRedLabel.PIXEL, HEIGHT = 40;
		shopcart.setBounds(0 - PIXEL, 615 - HEIGHT + PIXEL, 120, HEIGHT);
		myCommo.setBounds(120 - 2 * PIXEL, 615 - HEIGHT + PIXEL, 150, HEIGHT);
		myShoppingReport.setBounds(270 - 3 * PIXEL, 615 - HEIGHT + PIXEL, 150, HEIGHT);
		myShoppingDetail.setBounds(420 - 4 * PIXEL, 615 - HEIGHT + PIXEL, 150, HEIGHT);
...
//				{ "2021-06-25", "奶酪口味威化饼干", "食品&饮料", "正规商家", 3, 10.5 } };
	...
	public void listenerMyShoppingReport() {
		...
		int reportX = 360, reportY = 3666;

		JFrame frame = new JFrame();
		frame.setBounds(650 - (reportX + 34) / 2, 350 - 250, reportX + 34, 500);
		...
		goThrough.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) { // 单击了确定按钮
					// 获取在文件对话框中选择的文件
					imageURL.setText(chooser.getSelectedFile().getAbsolutePath());
				}
			}
...
//		panel.add(dateIn);
	...
	public void listenerCampusBank() {
		FunctionPanel jp = new FunctionPanel(jf, "校园银行");
...
//		double netBalanceValue = getNetBalance();
		...
		// [start], middle
		// [start], elements
		JLabel title = new JLabel("校园银行 - 一卡通余额管理系统");
...
//				new Course("数据库原理及应用（全英文）", "090003", "213193904", 3, 8, 9, 5, 8, 9, 1, 100, 96, "j4-102"), };
		...
		// 接口，返回给我所有课程
		CourseScrollContent courseInfo = new CourseScrollContent(courseList, person, timetable);
...
//						new Course("数据库原理及应用（全英文）", "090003", "213193904", 3, 8, 9, 5, 8, 9, 1, 100, 96, "j4-102"), };
				// TODO: 搜索
				courseScroll.setViewportView(new CourseScrollContent(courseListNew, person, timetable)); // 刷新列表
