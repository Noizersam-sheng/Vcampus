package ui.support.library;

/**
 * @author 牟倪
 * @version 2.1
 * 渲染馆藏信息的表格，需要与LibraryButtonRender配合使用
 */
...
public class JCollectionTable extends JTable {
	// collection是馆藏！borrow是借书！
	// collection（这个类）才是馆藏！！
	...
	public JCollectionTable(Object[][] content, JCButton fundam) {
		...
		for (int i = 1; i < this.getColumnCount(); i++) {
			this.setDefaultRenderer(this.getColumnClass(i), new RoutineTableCollection());
		}

		// add the buttons
		DefaultTableModel myModel = (DefaultTableModel) this.getModel();
		myModel.addColumn("借阅");
		this.getColumnModel().getColumn(5).setCellEditor(new LibraryButtonRender("借阅", fundam)); // 设置编辑器，不能注释掉
		this.getColumnModel().getColumn(5).setCellRenderer(new LibraryButtonRender("借阅", fundam));
		...
		this.getColumnModel().getColumn(4).setPreferredWidth(75);
		this.getColumnModel().getColumn(5).setPreferredWidth(69);
	}
...
class RoutineTableCollection extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
		...
		if (row == JCollectionTable.current) {
