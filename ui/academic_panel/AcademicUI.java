...
import ui.support.BackgroundTabbedFrame;
import ui.support.JCButton;
import ui.support.MyType;
import ui.support.PersonLabel;
import ui.support.SidebarPanel;
import ui.support.TranslucPanel;

public class AcademicUI {
	// figure & icon
	ImageIcon background = new ImageIcon(MyType.figurePath + "seubg2-cast-1200-650.jpg");
	static int X = 1200, Y = 650, X1 = 300, X2 = 900;	ImageIcon studentIcon = new ImageIcon(MyType.iconPath + "student.png");
	ImageIcon teacherIcon = new ImageIcon(MyType.iconPath + "teacher.png");
	...
	// big button
	JCButton studentButton = new JCButton("添加学生账号", studentIcon,jf,null);
	JCButton teacherButton = new JCButton("添加教师账号", teacherIcon,jf,null);

	// side bar element
	PersonLabel perLabel = new PersonLabel("教务老师", adminIcon);
	...
	public AcademicUI() {
		...
		jf.setTitle("VCampus - 东南大学虚拟校园 - 教务老师");
		...
		int basex = 200, basey = 280, spacex = 250, widthx = 120, heighty = 100;
		studentButton.setBounds(basex, basey, widthx, heighty);
		teacherButton.setBounds(basex+spacex, basey, widthx, heighty);
		...
		grid.add(studentButton);
		grid.add(teacherButton);
